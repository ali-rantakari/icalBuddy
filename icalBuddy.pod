...
Print events occurring between the two specified dates (START and END), where both are specified in the format C<YYYY-MM-DD HH:MM:SS +HHMM> (the C<+HHMM> part at the end specifies the timezone offset in hours and minutes from GMT, and it may have a plus or minus prefix)
...
Print uncompleted tasks that are due before the given date. DATE may be C<today>, C<tomorrow> or a date in the format C<YYYY-MM-DD HH:MM:SS +HHMM> (the C<+HHMM> part at the end specifies the timezone offset in hours and minutes from GMT, and it may have a plus or minus prefix)
...
Format dates included in the output based on the I<format> formatting string. See C<http://tinyurl.com/b9mgyp> [apple.com] for documentation on the syntax to use. (note: in addition to the standard datetime element specifiers, you can also use the S<C<%RW>> specifier, which stands for "relative week" (e.g. "next week"))
