...
[B<-Vufnd>] [B<--strEncoding> I<encoding>] [B<-sc>] [B<-sd>] [B<-nc>] [B<-nrd>] [B<-npn>] [B<-n>] [B<-sed>] [B<-uid>] [B<-eed>] [B<-li> I<num>] [B<-std>] [B<-stda>] [B<-tf> I<format>] [B<-df> I<format>] [B<-po> I<value>] [B<-ps> I<value>] [B<-b> I<value>] [B<-ab> I<value>] [B<-ss> I<value>] [B<-ic> I<value>] [B<-ec> I<value>] [B<-iep> I<value>] [B<-eep> I<value>] [B<-itp> I<value>] [B<-etp> I<value>] [B<-cf> I<path>] [B<-lf> I<path>] [B<-nnr> I<value>] I<command>
...
Print events occurring between the two specified dates. The dates (START and END) may be specified in a natural language form (such as C<tomorrow at noon> or C<june 10 at 6 pm>) but the safest format is C<YYYY-MM-DD HH:MM:SS +HHMM> (the C<+HHMM> part at the end specifies the timezone offset in hours and minutes from GMT, and it may have a plus or minus prefix)
...
Print uncompleted tasks that are due before the given date. DATE may be C<today+NUM> (where C<NUM> is an integer) or it may be specified in a natural language form (such as C<tomorrow at noon> or C<june 10 at 6 pm>) but the safest format is C<YYYY-MM-DD HH:MM:SS +HHMM> (the C<+HHMM> part at the end specifies the timezone offset in hours and minutes from GMT, and it may have a plus or minus prefix)
...
Format times included in the output based on the I<format> formatting string. See C<http://tinyurl.com/b9mgyp> [apple.com] for documentation on the syntax to use.

=item B<-df>, B<--dateFormat> I<format>

Format dates included in the output based on the I<format> formatting string. See C<http://tinyurl.com/b9mgyp> [apple.com] for documentation on the syntax to use. (note: in addition to the standard datetime element specifiers, you can also use the S<C<%RW>> specifier, which stands for "relative week" (e.g. "next week") and the S<C<%RD>> specifier which stands for "relative day" (e.g. "5 days from now").

=item B<-po>, B<--propertyOrder> I<value>

Print properties of events/tasks in the order specified by I<value>, which must be a comma-separated list of property names. The allowed property names are: title, location, notes, url, datetime, priority.
...
Sets I<value> as the string to be used as the bullet point in the output. The default is S<C<* >>.

=item B<-ab>, B<--alertBullet> I<value>

Sets I<value> as the string to be used as the alert bullet point in the output. The default is S<C<! >>.
...
Includes only items in calendars specified by I<value> into the output. I<value> must be a comma-separated list of calendar UIDs (which you can find out by using the 'calendars' I<command> parameter.) You can also use titles, but remember that they may not be unique (as opposed to UIDs.) The B<-ic> and B<-ec> parameters will be handled in the order of first include, then exclude.

=item B<-ec>, B<--excludeCals> I<value>

Excludes items in calendars specified by I<value> from the output. I<value> must be a comma-separated list of calendar UIDs (which you can find out by using the 'calendars' I<command> parameter.) You can also use titles, but remember that they may not be unique (as opposed to UIDs.) The B<-ic> and B<-ec> parameters will be handled in the order of first include, then exclude.

=item B<-iep>, B<--includeEventProps> I<value>

Includes only event properties specified by I<value> into the output. I<value> must be a comma-separated list of property names. Possible property names are: location, url, notes, datetime, title.
